...
# -----------------------------
# 小工具：圖表標題（固定英文；App → and st / 問卷 → Survey）
# -----------------------------
def chart_title(label: str, year: int) -> str:
    label_en = {"APP": "and st", "アンケート": "Survey"}.get(label, str(label))
    return f"{label_en} Monthly totals ({year})"
