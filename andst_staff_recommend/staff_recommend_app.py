...
def show_statistics(category: str, label: str):
    ...
    render_kpi_row([
        ...
        (f"w{selected_week_num:02d}週累計", f"{weekly_total}", "件", 
arrow = "▲" if delta_pct:+.1f > 0 else "▼" if delta_pct:+.1f < 0 else "→"
color = "green" if delta_pct:+.1f > 0 else "red" if delta_pct:+.1f < 0 else "gray"
f"前週 {prev_total}件 ({arrow} {delta_pct:+.1f:.1f}%)"
),
    ])
    if week_range_text:
