...
def weekly_progress_chart(df: pd.DataFrame, category: str = "app", theme: str = "dark"):
    ...
    if category == "app":
        fig.add_bar(name="新規", x=df["week_label"], y=df["new"], marker_color=c["new"], **bar_kwargs)
        fig.add_bar(name="既存", x=df["week_label"], y=df["exist"], marker_color=c["exist"], **bar_kwargs)
        fig.add_bar(name="LINE", x=df["week_label"], y=df["line"], marker_color=c["line"], **bar_kwargs)
    else:
        fig.add_bar(name="アンケート", x=df["week_label"], y=df["survey"], marker_color=c["new"], **bar_kwargs)
    ...
    fig.add_trace(
        go.Scatter(
            ...
            yaxis="y2",
            hovertemplate="%{x}<br>目標: 100%<extra></extra>",
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            cliponaxis=False,
            yaxis="y2",
            hovertemplate="%{x}<br>達成率: %{y:.1f}%<extra></extra>",
        )
    )
