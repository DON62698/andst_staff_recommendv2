...
def build_weekly_progress_df(df_month: pd.DataFrame, monthly_target: int, category: str) -> pd.DataFrame:
    ...
    grouped["week_label"] = grouped["iso_week"].astype(int).apply(lambda x: f"w{x:02d}")
...
def show_statistics(category: str, label: str):
    ...
    if week_range_text:
        st.caption(f"対象週: {selected_week_label} / {week_range_text}　※完整週ベースで集計")
    ...
    st.dataframe(
        daily[["label", "count"]].rename(columns={"label": "曜日", "count": "合計"}),
    ...
    if category == "app":
        ...
        if total > 0:
            ...
            ax.set_title("構成比（新規・既存・LINE）", color=pie_fg)
            for t in autotexts:
                t.set_color(pie_fg)
                t.set_fontsize(10)
    ...
    if df_staff.empty:
        ...
        st.markdown("#### TOP3")
        top3 = staff_sum.head(3).copy()
        if not top3.empty:
            top_cols = st.columns(min(3, len(top3)))
            for c, (_, row) in zip(top_cols, top3.iterrows()):
                with c:
                    render_kpi_row([(f"{row['順位']}位", f"{int(row['合計'])}", "件", str(row['スタッフ']))])
    ...
    if df_year.empty:
        ...
        ax.set_title(f"{title_label} 月別累計（{int(year_sel3)}）", color=bar_fg)
